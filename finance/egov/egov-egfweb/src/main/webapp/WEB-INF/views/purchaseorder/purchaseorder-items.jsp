...
<div class="panel-heading">
	<div class="panel-title">
		<spring:message code="lbl.purchase.items" /> 
	</div>
</div>
...
<input type="hidden" value="${purchaseOrder.purchaseItems.size()}" id="debitAmountrowcount"/>
	...
	<thead>
			<tr>
				<th><spring:message code="lbl.items"/></th>
				<th><spring:message code="lbl.unit"/></th>
				<th><spring:message code="lbl.unit.rate"/></th>
				<th><spring:message code="lbl.gst.rate"/></th>
				<th><spring:message code="lbl.unit.value.with.gst"/></th>
		...
		<c:when test="${purchaseOrder.purchaseItems.size() == 0}">
			<form:hidden path="purchaseItems[0].glcodeid" id="purchaseItems[0].glcodeid" class="form-control table-input hidden-input debitdetailid"/> 
				...
				<form:hidden path="purchaseItems[0].glcodeId.id" name=rchaseItems[0].glcodeId.id" id="purchaseItems[0].glcodeId.id" class="form-control table-input hidden-input accountglcodeid"/> 
			...
			<td class="text-center"><span style="cursor:pointer;" onclick="addDebitDetailsRow();"><i class="fa fa-plus" aria-hidden="true"></i></span>
						 <span class="add-padding purchaseitems-delete-row" onclick="deleteDebitDetailsRow(this);"><i class="fa fa-trash"  aria-hidden="true" data-toggle="tooltip" title="" data-original-title="Delete!"></i></span> </td> 
			</tr>
			</c:when>
			 <c:otherwise>
			...
			<td>
			<input type="text" id="purchaseItems[${item.index}].itemCode" name="purchaseItems[${item.index}].itemCode" class="form-control table-input debitDetailGlcode itemCode"  data-errormsg="Account Code is mandatory!" data-idx="0" data-optional="0"   placeholder="Type first 3 letters of Account code" value="${billDeatils.chartOfAccounts.glcode }"/>
			 ...
			 --%></td>
			<td>
				<select id="purchaseItems[${item.index}].unit" class="form-control table-input debitdetailname unit">
					<option value="No">No</option>
					<option value="No">No</option>
				</select>
			</td>
...
$(document).on('input', '.unitRate, .gstRate, .quantity', function () {
       ...
       //var unitValueWithGst = parseInt($(this).closest('tr').find('.unitValueWithGst').val()) || 0;
        console.log("Rajveer-------------------- gstRate: " + gstRate);
        //calculate unitValuewithGst
         var unitValueWithGst= ((unitRate* gstRate) / 100) + unitRate;
         
        console.log("Rajveer---------------- unitValueWithGst: "+unitValueWithGst);
