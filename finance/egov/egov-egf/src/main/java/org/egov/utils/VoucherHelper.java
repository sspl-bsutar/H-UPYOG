...
public class VoucherHelper {
	...
	public static String sequenceNameFor(final String voucherType, final String fiscalPeriodName) {
		return new StringBuilder().append(DEFAULT_SEQUENCE_PREFIX).append(voucherType).append(UNDERSCORE)
	...
	public String getGeneratedVoucherNumber(final Long fundId, String voucherType, final Date voucherDate,
		...
		if (vNumGenMode.equalsIgnoreCase("Auto")) {
			...
			final ScriptContext scriptContext = ScriptService.createContext("fundIdentity", fundIdentifier,
					"voucherType", voucherType, "transNumber", transNumber, "vNumGenMode", vNumGenMode, "date",
					voucherDate, "month", month, "commonsService", financialYearDAO, "dbSequenceGenerator",
					databaseSequenceCreator, "sequenceNumberGenerator", genericSequenceNumberGenerator, "voucherNumber",
					voucherNumber, "sequenceName", sequenceName);
